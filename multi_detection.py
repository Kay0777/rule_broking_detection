...
def postprocess(
    ...
    pred_masks: np.ndarray = None,  # type: ignore
    ...
    for i, pred in enumerate(preds):
        ...
        if retina_mask:
            ...
            segments = [ops.scale_segments(input_hw, x, shape, normalize=False) for x in ops.masks2segments(masks)]  # type: ignore
        else:
...
def handle_traffic_light_detection(image_draw: np.ndarray, bbox: tuple, frame: np.ndarray) -> np.ndarray:
...
def final_process(frame: np.ndarray, detection: list, i: int, results: list) -> None:
    start = time.time()
    ...
    outputs = get_result(detection, class_dict)  # type: ignore
    end = time.time()
    diff = (end - start) * 1000
    ...
    if rightmost_traffic_light_bbox is None:
        ...
        tl_image = handle_traffic_light_detection(image_draw, rightmost_traffic_light_bbox, frame)  # type: ignore
    ...
    if car_detections:
        ...
        for tracked_obj in car_tracked_objects:
            ...
            if all(coord is not None for coord in [px1, py1, px2, py2]):
                if x1 <= px1 <= x2 and y1 <= py1 <= y2 and x1 <= px2 <= x2 and y1 <= py2 <= y2:  # type: ignore
...
def tl_color(frame: np.ndarray, analyzed_frame: np.ndarray) -> str:
...
def process_frame(frames: list[np.ndarray], tensor: list[np.ndarray]) -> list:
    ...
    detections = postprocess(
        pred_boxes=boxes,
        input_hw=corrected_tensor.shape[2:],  # type: ignore
        orig_img=frames,  # type: ignore
        pred_masks=None  # type: ignore
    )
    end = time.time()
    diff = (end - start) * 1000
    ...
    for i in range(len(boxes)):
        thread = Thread(target=final_process, args=(frames[i], detections[i], i, analyzed_frames))
        thread.start()
    ...
    for frame, analyzed_frame in zip(frames, analyzed_frames):
        if isinstance(analyzed_frame[2], str) and analyzed_frame[2] == "None":  # type: ignore
            continue
        analyzed_frame[2] = tl_color(frame=frame, analyzed_frame=analyzed_frame[2])  # type: ignore
    end = time.time()
    diff = (end - start) * 1000
