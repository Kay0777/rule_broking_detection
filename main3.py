...
def analyze_active_frame(active_frames: dict, lines_info: list, analyzed_frames: list):
    ...
    while True:
        ...
        if active_frames:
            ...
            print(len([tensor[3] for tensor in sorted_frames]))
                ...
                if len([ips[0] for ips in sorted_frames]) > 1:
                    if len([tensor[3] for tensor in sorted_frames]) == 2:
                        ...
                        with ThreadPoolExecutor() as pool:
                            pool.submit(process_frame, [frame[2] for frame in sorted_frames], [tensor[3] for tensor in sorted_frames])
...
def main():
    ...
    # #     thread.join()
    # analysis_thread = threading.Thread(target=analyze_active_frame, args=(active_frames, lines_info, analyzed_frames))
    # analysis_thread.start()
        # analysis_thread.join()
    with Manager() as manager:
        active_frames = manager.dict()
        analyzed_frames = manager.list()
        ...
        with ProcessPoolExecutor() as pool:
            ...
            analyze_processes = []
            # for cameraIP in cameraIPs:
            analyze_process = multiprocessing.Process(target=analyze_active_frame, args=(active_frames, lines_info, analyzed_frames))
